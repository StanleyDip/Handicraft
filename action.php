		...
		while($row=mysqli_fetch_array($run_query)){
			...
			if(isset($_POST["get_cart_product"])){
				...
				<div class='row'>
					...
					<div class='col-md-3 col-xs-3'>tk.$pro_price.00</div>
				</div>
			";
			$no = $no + 1;
			}else{
...
///For Maker tools and Watch video
...
if(isset($_POST["category"])){
	...
	echo "
		<div class='nav nav-pills nav-stacked'>
			<li class='active'><a href='#'><h4>Video Categories</h4></a></li>
	";
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			$cid = $row["cat_id"];
...
// if(isset($_POST["brand"])){
// 	$brand_query = "SELECT * FROM videos";
// 	$run_query = mysqli_query($con,$brand_query);
// 	echo "
// 		<div class='nav nav-pills nav-stacked'>
// 			<li class='active'><a href='#'><h4>Videos</h4></a></li>
// 	";
// 	if(mysqli_num_rows($run_query) > 0){
// 		while($row = mysqli_fetch_array($run_query)){
// 			$bid = $row["vid_id"];
// 			$brand_name = $row["vid_title"];
// 			echo "
// 					<li><a href='#' class='selectBrand' bid='$bid'>$brand_name</a></li>
// 			";
// 		}
...
if(isset($_POST["getProduct"])){
	...
	$product_query = "SELECT * FROM videos LIMIT $start,$limit";
	$run_query = mysqli_query($con,$product_query);
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			$pro_id    = $row['vid_id'];
...
if(isset($_POST["get_seleted_Category"]) || isset($_POST["search"])){
	if(isset($_POST["get_seleted_Category"])){
		$id = $_POST["cat_id"];
		$sql = "SELECT * FROM videos WHERE vid_cat = '$id'";
	}
	// }else if(isset($_POST["selectBrand"])){vid
	// 	$id = $_POST["brand_id"];
	// 	$sql = "SELECT * FROM videos WHERE vid_brand = '$id'";
	// }
	else {
		$keyword = $_POST["keyword"];
		$sql = "SELECT * FROM videos WHERE vid_title LIKE '%$keyword%'";
	}
			...
			echo "
				<div class='col-md-8' >
							<div class='panel panel-info'>
								<div class='panel-heading'>$pro_title</div>
								<div class='panel-body'>
									<video width='500' controls><source src='product_images/".$pro_image."' type='video/webm'></video>
								</div>
